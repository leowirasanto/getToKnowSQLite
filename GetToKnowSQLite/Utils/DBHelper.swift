...
class DBHelper {
    ...
    let dbPath: String = "getToKnowDb.sqlite"
    ...
    func createTable() {
        ...
        if sqlite3_prepare_v2(db, createTableString, -1, &createTableStatement, nil) == SQLITE_OK {
            if sqlite3_step(createTableStatement) == SQLITE_DONE {
                ...
                print("person table couldn't be created")
            }
        } else {
            print("CREATE TABLE statement couldn't be prepared")
        }
    ...
    func insert(id: Int, name: String, age: Int) {
        let persons = read()
        for p in persons {
            //avoid replacing new data with same id
            if p.id == id {
                return
            }
        }
    ...
    func read() -> [Person] {
        ...
        if sqlite3_prepare_v2(db, queryStatementString, -1, &queryStatement, nil) == SQLITE_OK {
            while sqlite3_step(queryStatement) == SQLITE_ROW {
                ...
                persons.append(Person(id: Int(id), name: name, age: Int(year)))
                print("query result:")
                print("\(id) | \(name) | \(year)")
            }
        } else {
